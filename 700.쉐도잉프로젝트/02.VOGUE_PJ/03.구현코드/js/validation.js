...
form.logF input[type=password]`).blur(function () {
    ...
    ******************************************/
    ...
    // 정규식은 슬래쉬 사이에 쓰며 \s는 스페이스 기호,g는 전역 플래그이다.
    // 이름일 경우 중간공백 없애기 제외
    cv = cid == "mnm" ? cv.trim() : groSpace(cv);
    $(this).val(cv);
    console.log("현재 아이디는?", cid, "입력한 값은?", cv);
    /************************************* 
        3. 빈값 여부 검사하기 (필수입력항목)
    *************************************/

    if (cv == "") {
      $(this).siblings(".msg").text("필입");
    } //////if////////
    ...
    ****************************************/
      ...
      if (!vReg(cv, cid)) {
        ...
        $(this).siblings(".msg").text("멋져").addClass("on");
      }
    } //////////else if////////
    /**************************************** 
        5. 비밀번호일 경우 유효성 검사
