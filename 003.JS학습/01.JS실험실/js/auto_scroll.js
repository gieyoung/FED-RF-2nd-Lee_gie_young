...
// 1. 함수이름을 대문자로 시작
// 2. 호출시 new키워드를 사용하여 인스턴스를 생성
// -> 일반함수와 차이점은 개별적인 객체 인스턴스로 함수가 개별화되어
// 운용상 독립적인 프로페스를 확보하게 됨
// this키워드를 사용하는 경우는 new로 생성된 인스턴스객체에서
// 함수 내부에 있는 변수나 함수를 직접 호출해야 할 경우 사용한다.

// 자동스크롤 기능 함수
export default function AutoScrollFn() {
  // 기본 초기화 css설정
  // html - 부드러운 스크롤(스크롤 애니메이션 설정)
...
*********************************************************/
  ...
  function touchEndFn(e) {
    ...
    // 모바일 이벤트값 객체는 touches[0]임
    // 그러나 같은 이벤트가 연속될 경우 변경된 값을 읽어와야 하므로
    // changedTouches[0]를 사용해야함

    // 처음 터치위치와 마지막 위치의 차 구하기
    let diffValue = mPosEnd - mPosStart;
    console.log(mPosStart, mPosEnd, diffValue);

    // 양수는 위에서 아래로 쓸어내릴 때 (아랫페이지로 이동)
    // 음수는 아래에서 위로 올라갈 떄(윗페이지로 이동)
