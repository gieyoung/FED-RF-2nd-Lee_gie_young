...
function slideFn(selEl) {
  ...
  *******************************************************/
  function coverDrag() {
    // selEl로 전달된 대상에 클래스 on을 줘서 가상요소로 셋팅된 슬라이드 커버가 나오게 함
  ...
  abtn.forEach((ele) => ele.addEventListener("mouseenter", moveDragSlide));
  ...
  mFn.qsaEl(selEl,'.control').forEach(
    (ele) =>
